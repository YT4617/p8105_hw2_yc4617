...
```{r}
data$Entry <- case_match(data$Entry,"YES" ~ TRUE, "NO" ~ FALSE)
head(data)
```
## Describe the dataset
The data set contains these variables: `r paste(names(data), collapse = ", ")`. Until now, I read the data, then convert Entry variable from charater to logical variable, and show the variables' names. The resulting data set has `r dim(data)[1]` rows and `r dim(data)[2]` columns. These data are not tidy because there are some NA in column Route 8 - Route 11.
...
Mr_Trash_wheel = read_excel("202309 Trash Wheel Collection Data.xlsx",sheet = "Mr. Trash Wheel") %>%
...
Season_1_avg <- mean(Viewers$"Series 1", na.rm = TRUE)
Season_5_avg <- mean(Viewers$"Series 5", na.rm = TRUE)
```
